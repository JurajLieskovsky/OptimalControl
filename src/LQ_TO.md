...
\end{gathered}
$$
at which point the problem is still classified as a quadratic program. In this case, without the knowledge of constrained optimization method, we can either use [optimization modeling languages](OptimizationModelingLanguages.md) or [use sparse solvers for QP](MPC-Direct.md). Alternatively, if constrained optimization methods are applied, they generally form a sequence of problems similar to (2) which can be solved using the Riccati recursion.
